...
# class PredictionResponse(BaseModel):
#     results: List[StockPredictionResponse]
...
#         test_data = scaled_data[training_data_len - seq_length:]
#         x_test = create_dataset(test_data, seq_length)
#         last_100_days = scaled_data[-seq_length:] if len(scaled_data) >= seq_length else np.vstack((np.zeros((seq_length - len(scaled_data), 1)), scaled_data))
...
# # Helper function to create datasets
# def create_dataset(data, seq_length=100):
#     return np.array([data[i - seq_length:i] for i in range(seq_length, len(data))])
...
#             test_predictions = scaler.inverse_transform(test_predictions).flatten().tolist()
#             data.reset_index(inplace=True)
#             test_dates = data['Date'].dt.strftime('%Y-%m-%d')[-len(test_predictions):].tolist()
#             test_predictions_dict = dict(zip(test_dates, test_predictions))
...
#             future_predictions = scaler.inverse_transform(np.array(future_predictions).reshape(-1, 1)).flatten().tolist()
#             future_dates = [(last_date + pd.Timedelta(days=i)).strftime('%Y-%m-%d') for i in range(1, request.days + 1)]
#             future_predictions_dict = dict(zip(future_dates, future_predictions))
#             print('future=',future_predictions_dict)
#             print('tp=',test_predictions_dict)
            
#             results.append(
#                 StockPredictionResponse(
#                     stock=stock,
#                     closing_prices={pd.to_datetime(date).strftime('%Y-%m-%d'): price for date, price in closing_prices.items()},
#                     test_predictions=test_predictions_dict,
